...
int write_screen_to_file(const char *filename, WINDOW *win)
{
	...
	if (ofile)
	{
		...
		fprintf(ofile, "</head><body><p><tt>\n");
		for (y = 0; y < maxy; y++)
		{
			mvwinchnstr(win, y, 0, buff, maxx);
			for (x = 0; x < maxx; x++)
			{
				unsigned ival = (buff[x] & A_CHARTEXT);
...
# endif
	...
	while (!quit)
	{
		...
		else if (c >= KEY_F(4) && c < KEY_F(12))
		{
			...
			write_screen_to_file("scrdump.htm", curscr);
		if (c != KEY_MOUSE)
		{
